...
impl Qwen3Embedder {
    pub fn local(model_path: &str, dtype: Option<crate::Dtype>) -> Result<Self, anyhow::Error> {
        let config_file = format!("{}/config.json", model_path);
        let tokenizer_file = format!("{}/tokenizer.json", model_path);
        let weights_file = format!("{}/model.safetensors", model_path);
        let (config_filename, tokenizer_filename, weights_filename) = (
            Path::new(&config_file),
            Path::new(&tokenizer_file),
            Path::new(&weights_file),
        );
...
mod tests {
    ...
    fn test_qwen3_embed() {
        let embedder =
            Qwen3Embedder::from_local_path("Qwen/Qwen3-Embedding-0.6B", Some(crate::Dtype::F32))
                .unwrap();
